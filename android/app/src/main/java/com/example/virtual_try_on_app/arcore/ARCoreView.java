...
public class ARCoreView extends ArSceneView {
    ...
    private final HandPointsOverlayView handPointsOverlayView;
    ...
    public ARCoreView(Context context) {
        ...
        handPointsOverlayView = new HandPointsOverlayView(context);
    ...
    public HandPointsOverlayView getHandPointsOverlayView() {
        return handPointsOverlayView;
    }
            ...
            if (session != null) {
                ...
                post(() -> {
                    try {
                        ...
                        System.err.println("Error unavailable camera during post-resume: " + e.getMessage());
                        postDelayed(() -> {
                            try {
                                if (session != null) {
                                    session.resume();
                                }
                            } catch (CameraNotAvailableException errorAfterRetry) {
                                System.err.println("Error unavailable camera after retry: " + errorAfterRetry.getMessage());
                            }
                        }, 100);
                    } catch (Exception e) {
                ...
                System.err.println("Error null Session");
                resume();
            }
        } catch (CameraNotAvailableException e) {
            System.err.println("Error unavailable camera: " + e.getMessage());
            e.printStackTrace();
        } catch (Exception e) {
    ...
    private void updateAnchor(float x, float y) {
        ...
        try {
            ...
            System.err.println("Hit test failed: " + e.getMessage());
        }
    ...
    public void drawhand(List<NormalizedLandmark> landmarks) {
        ...
        List<float[]> all_points = new java.util.ArrayList<>(landmarks.size()+1);
        for (int i = 0; i < landmarks.size(); i++) {
            float x = landmarks.get(i).x() * viewWidth;
            float y = landmarks.get(i).y() * viewHeight;
            all_points.add(new float[]{x, y});
        }
        handPointsOverlayView.replaceAll(all_points);
    }
    ...
    public void clearPoints() {
        handPointsOverlayView.clearPoints();
    }
